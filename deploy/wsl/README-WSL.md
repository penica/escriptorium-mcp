# eScriptorium MCP 1.1.0: WSL transfer bundle
...
For a specific release, append `-s -- 1.1.0` after `sh`. Downgrades are refused.
...
```bash
mkdir -p ~/mcp-updates/1.1.0
cd ~/mcp-updates/1.1.0
curl -fL -O https://github.com/penica/escriptorium-mcp/releases/download/v1.1.0/escriptorium-mcp-1.1.0-wsl.tar.gz
curl -fL -O https://github.com/penica/escriptorium-mcp/releases/download/v1.1.0/escriptorium-mcp-1.1.0-wsl.tar.gz.sha256
sha256sum --check escriptorium-mcp-1.1.0-wsl.tar.gz.sha256 && tar -xzf escriptorium-mcp-1.1.0-wsl.tar.gz
```

After successful verification/extraction, stop the existing service and upgrade:

```bash
cd ~/mcp-updates/1.1.0/escriptorium-mcp-1.1.0-wsl
...
Reconnect MCP clients after the upgrade to refresh the 176-tool catalogue.
...
```bash
cd ~
tar -xzf /mnt/c/Users/YOUR_WINDOWS_USER/Downloads/escriptorium-mcp-1.1.0-wsl.tar.gz
cd escriptorium-mcp-1.1.0-wsl
```
...
It verifies bundled files and installs locked dependencies with Python 3.11. Running it again upgrades an older installation or reconfigures version 1.1.0, backs up its configuration, and preserves the HTTP token and archive settings. It asks before replacing an existing service. Credentials are stored with owner-only permissions and are not printed.
...
Version 1.1.0 exposes 176 tools. It adds direct script lookup and optional one-page retrieval on supported list tools while preserving complete retrieval when pagination is omitted. See `docs/PAGINATION.md` for the exact bounded-read contract. The 1.0 feature set remains available, including fonts, sharing, witnesses, collections, metadata, imports/exports, segmentation, transcription, models, jobs and ontology operations. Training groups remain unproven candidates; model idle state is not successful completion. Model writes require ownership, and overwrite/replacement/deletion requires stopped training. Document associations are read-only in the audited REST API. See `IMPLEMENTATION-STATUS.md` for release validation. The maintainer confirmed the existing WSL service, authentication and client connection before 1.0.0 preparation; verify these again after upgrading your own installation. Nothing is deployed automatically by transferring or extracting the archive.
