...
if [ ! -e "$install_dir/.venv" ]; then
    ...
    uv pip install --python "$install_dir/.venv/bin/python" --no-deps escriptorium_mcp-0.15.0-py3-none-any.whl
else
    installed_version="$("$install_dir/.venv/bin/python" -c 'from importlib.metadata import version; print(version("escriptorium-mcp"))')"
    if [ "$installed_version" != '0.15.0' ]; then
        ...
        echo "Upgrading $installed_version to 0.15.0; saved configuration will be retained."
        uv pip install --python "$install_dir/.venv/bin/python" --require-hashes -r requirements.txt
        uv pip install --python "$install_dir/.venv/bin/python" --no-deps escriptorium_mcp-0.15.0-py3-none-any.whl
    else
        echo 'Existing 0.15.0 installation found; configuring it again.'
    fi
fi
