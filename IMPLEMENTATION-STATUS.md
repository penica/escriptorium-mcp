...
- Native group creation requires exact true acknowledgment of missing creator membership/ownership in pinned upstream source. One accepted POST remains accepted even if its single diagnostic read fails or returns incomplete/malformed evidence. Unknown membership/ownership observations remain null. No repair, rollback or automatic resubmission occurs. Existing group edits follow native member visibility rather than an invented owner-only rule.
- Sharing grants accept exactly one username or member-group target. Scoped resource IDs and group visibility are checked; username sharing does not require directory discovery. Native additive responses and existing grants are preserved. Unsupported revoke/replace, roles, expiry, membership, owner-transfer, password and privilege operations are not offered.
- Initial actual-STDIO RED tests proved missing account/group/sharing tools. Additional group/sharing failures exposed SDK-generated top-level argument models silently ignoring unknown fields, allowing an unintended additive grant or ownerless group creation. A narrow public Tool factory now sets extra=forbid, rebuilds runtime validation and advertises additionalProperties=false for all 13 new tools before public MCPServer constructor registration. Existing 160 tools retain their registration behavior; no private SDK manager access or global monkeypatch is used.
- All 158 new cases passed after the strict-boundary fix: 61 account cases in 50.80 seconds, 49 group cases in 73.14 seconds and 48 sharing cases in 46.40 seconds. They exercise STDIO/authenticated HTTP, native field limits and permissions, unknown-input zero-network rejection, local directory filtering/pagination, nullable creation diagnostics, one-write/one-readback boundaries and raw additive responses. Original failing cases were retained. Nine existing transport cases passed after integration in 6.30 seconds. The complete suite collects 1339 tests for exact-commit platform CI.
- Ruff lint/format, strict Basedpyright, changed-file programming checks across 32 Python files, shell syntax and skill validation passed. Live read-only STDIO verified 0.17.0/173 tools, current/own identity, one visible user, an empty group catalogue, local filters, missing-group errors and acknowledgment validation. Evidence stores shapes/counts rather than personal values. No account, email, group, share, invitation, credential or service was changed.
...
### Cross-platform validation correction before 0.18.0 publication
...
### First-install and cross-platform skill scripts — 2026-09-16

- Added `install.sh` for a first WSL/Linux installation with checksum verification, automatic uv setup when missing, and URL/key/transport/service prompts read from the controlling terminal during piped execution. Existing environments are refused in favor of the configuration-preserving updater.
- Added `install-skills.sh` and `install-skills.ps1`; rerunning either updates the release-matched eScriptorium skill and bundled references. New installs use the documented user skills directory, existing legacy destinations are retained, identical content is a no-op, and replaced local edits are backed up outside scanned skill directories. No MCP credentials or client connection configuration is changed.
- [Installation run 35132627203](https://github.com/penica/escriptorium-mcp/actions/runs/35132627203) passed all seven jobs at exact commit `4dd2c70398420c197c6f8ccc153b77fa35332562`: shell lifecycle tests on Linux/macOS; real-release PowerShell lifecycle tests on Windows PowerShell 5.1 and PowerShell 7 on Windows/Linux/macOS; and a Linux pseudo-terminal first install. The Linux bootstrap installed uv from its official installer and the actual 0.18.0 release, verified installed version/help, and refused a second first-install attempt.
- Local ShellCheck/syntax and real-release shell/PowerShell skill installation passed. The terminal bootstrap was also exercised locally with its Linux platform check simulated. All destinations were isolated; no user skill, live service or eScriptorium record was changed. Service creation uses the existing release installer; the new Linux bootstrap test selected STDIO rather than deploying a service.
- Initial CI exposed an unsupported workflow shell expression and a Unix PowerShell hidden-file cleanup error in the test fixture. Both were corrected without changing installer behavior or weakening assertions. The hidden-file failure was reproduced and reversed with `-Force`; production cleanup already had that flag. Published MCP 0.18.0 package bytes are unchanged.
