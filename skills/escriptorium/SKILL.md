...
- Creating or moving a document uses a project **slug**; choosing its writing system uses a script **name** from `list_scripts`, not its primary key. Use read-only `get_script(script_id)` when a known script ID needs its native detail record; it preserves the server mapping.
- `move_page` uses a **zero-based index**. Line ordering is separate from page ordering.

## Bounded list reads

Most public list tools expose optional `pagination`. Omit it when the request needs the established complete result: omission follows every native page and preserves the prior result shape. Supply `{"page": positive_integer, "page_size": optional_1_to_50}` to read exactly one native page; `page` defaults to 1. `page_size` is accepted only when that particular native route supports `paginate_by`; otherwise report the tool error instead of silently applying a local limit. Check the discovered tool schema for the currently supported list surface.

One-page responses retain native fields and add `pagination` metadata. Use its validated `next_page`/`previous_page`, never an arbitrary native link. The metadata reports native collection total separately from a local filtered total. With a local filter (`search`, task state/method, or download report ID), filtering applies only to the selected page: a sparse or empty page does not prove there are no matches elsewhere, and global `filtered_total` remains unknown.

Pagination is not a snapshot while projects, pages or jobs change. Do not use a bounded page to declare an audit, archive, register acquisition, submission-tracking lookup, or job summary complete; those internal operations continue to retrieve their complete scopes.
See the bundled `docs/PAGINATION.md` guide for route support, response fields and
...
- `list_tasks` supports document/group/date ordering and local state/exact-method filters. Without `pagination`, local filtering reads the complete visible task collection; with a selected page, it filters only that page and cannot report a global filtered total. `get_import_status` reads import task history and messages, optionally within a caller-selected `group_id`; it cannot provide native import processed/total counts. Import `track=true` also returns only unconfirmed candidates. No visible reports does not prove no import exists. Check finished reports for skipped-file warnings and separately queued image conversion.
...
Use `list_fonts` and `get_font` to inspect available font metadata; listing a font URL does not download or install it. Set `transcription_font` in existing project settings or document create/update inputs. A positive ID selects the font; explicit null clears an override, while omission preserves it. The tools require writable-field capability metadata even for clearing, so unsupported servers fail before a write.

Changing a project font affects documents that inherit it. Document overrides take precedence, followed by project, requesting-user preference and editor default. Read the server's `effective_transcription_font`; do not infer the effective result from the direct ID or claim clearing forces the default. Font selection changes presentation, not Unicode text, OCR, geometry or scan pixels. Font upload/metric editing belongs to admin; user preference belongs to the web profile because these setters are absent from this REST API. See the bundled `docs/FONTS-API.md` contract guide.

## Sharing, users and groups

Use current-user and directory tools to discover visible accounts/groups; nonstaff user discovery can be self-only, and groups are membership-scoped even for staff. Optional directory search is local over all visible pages when `pagination` is omitted; it applies only to the selected native page otherwise. `share_project` and `share_document` add one username/group grant while preserving existing grants. A known username need not appear in a nonstaff directory. Direct sharing arrays do not describe every source of access. The audited REST API cannot revoke/replace sharing, edit group membership or transfer ownership; do not use resource/account deletion as a substitute.

Account creation/deletion requires staff. Native self/staff edits support username, email, display names and active status. Account creation does not set a password or send an invitation; no credential fields are accepted. Self-deactivation/deletion can end access, and username changes can affect configured login and future shares. Do not repeat an accepted mutation just because a later read fails or change connection credentials automatically.

`create_group` requires `acknowledge_native_create_limitations=true`: the audited native create path can omit creator membership/ownership. Preserve accepted creation separately from its nullable verification observations. Failed verification does not authorize repair through other interfaces, deletion or another POST. Rename/delete of existing visible groups follows native permissions; deletion can remove access for all members. Do not claim password, invitation, privilege, quota or user-font operations absent from this API.

## Reference texts and alignment

Witnesses are owned reference files reused across documents. Upload/replacement paths belong to the MCP host and must contain nonempty UTF-8 `.txt` data. Standalone `upload_textual_witness` requires `acknowledge_unverified_ownership=true`: the audited backend can create ownerless inaccessible records. Preserve its native acceptance and separate ownership-readback result; an unavailable readback must not trigger another upload or a success claim about usable ownership. Do not launch alignment merely to store a reference. Updates/deletion can affect queued consumers; download needed bytes first when preservation is requested.

`align_document` uses an active source layer and either an owned witness ID or a direct file upload. Supply `acknowledge_target_reuse=true` only within authorization to write the named target: names can reuse hidden archived layers. The source name is prohibited. Offset search explicitly disables beam search; use the typed search variant. With `merge=false`, old unmatched target text remains; with merge true, copying can extend beyond selected region types. `full_doc=true` can read the full source document while writing only selected output pages. None of these operations provides a rollback snapshot.

`force_align_pages` replaces character graphs in the specified existing layer, including native-supported archived layers; it does not rewrite text or segmentation. A readable recognition model is required, but metadata does not prove architecture compatibility. Preserve queued acceptance, distinguish it from completion, and inspect method-filtered task reports. Only ordinary alignment has optional unconfirmed group tracking. There is no precise REST alignment-cancel operation; existing cancellation has broader effects and upstream limits.
...
On supported servers use `list_downloads`, optionally filtered locally by `task_report_id`, and `get_download` for owned metadata. With a selected download page, that local filter applies only to the selected page and cannot establish the global matching total. `download_generated_export` takes a 32-character lowercase hexadecimal fingerprint and a new MCP-host destination, verifies identity/size and returns bytes/SHA-256. It ignores advertised file URLs as routing authority and increments server access counters. Expired metadata can remain visible while file retrieval fails. The checksum proves downloaded bytes, not archive completeness. Existing files and partials are never overwritten, and exports stay outside Books.
